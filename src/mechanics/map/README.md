...
| `ai:decision-made` | `{ decision }` | On `EXPAND`: captures `decision.frame` for combat log turn labelling, then runs combat resolution against a random neighbouring province **owned by a country currently at war with the attacker** (truced/neutral neighbours are excluded from the candidate pool). Attacker uses `armies × morale + base 50 + flat roll`; defender uses `armies × terrain × morale + walls bonus 60 + base 20 + flat roll`. Morale is a per-side ±15% multiplier (0.85–1.15×) on raw army strength; flat roll is +0–30. Terrain multipliers: plains 1.0, hills 1.3, mountains 1.6, forest 1.2, tundra 1.1, desert 0.9. After outcome is determined, emits `military:casualties-taken` with per-army strength reductions for all participating armies (winner: 12–27%, loser: 28–48%, scaled by battle intensity). Attacker win → `map:province-conquered`; defender win → `map:province-attack-repelled`. Both outcome events carry `attackerStrengthLost` / `defenderStrengthLost` for combat log display. |
